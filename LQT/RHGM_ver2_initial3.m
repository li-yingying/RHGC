...
% initialization is method 3: track  optimal steady state
% Let z_{t+1}= x_{t+1}^{Index}(e), x(e)=Ax(e)+B u(e),
% thetat, xit
% At 2:39pm May 5 2019: this is the correct version, old version RHGM.m is wrong


function [z, x,u, Cost,gradz,K, z_ini] = RHGM_ver2_initial3(A, B, W,  epsilon, beta, delta, gamma,  x0, Q, R, theta, xi,N)
...
if K==0
    ...
    for t= 1-W:N-1
        ...
        if t+W<=N
            ...
            minimize((xe-theta(:,t+W))'*Q(:,:,t+W)*(xe-theta(:,t+W))+(ue-xi(:,t+W))'*R(:,:,t+W)*(ue - xi(:,t+W)))
            ...
            z_ini(:,t+W) = xe(Index); %note theta_t t=1,..., N+1 corresponds to paper's index: theta_0, ..., theta_N
